...
function _getKeyFromSpecialKeyMap(keyCode, $$char, specialKeyMap) {
  var key = MutableSparseMap$WonderEventmanager.get(specialKeyMap, keyCode);
  if (key !== undefined) {
...
function _handleShiftKey(keyCode, $$char, param) {
  var upperCaseChar = MutableSparseMap$WonderEventmanager.get(param[0], keyCode);
  if (upperCaseChar !== undefined) {
    return upperCaseChar;
  }
  var upperCaseChar$1 = MutableHashMap$WonderEventmanager.get(param[1], $$char);
...
function execEventHandle(eventName, keyboardDomEvent, po) {
  var arr = MutableSparseMap$WonderEventmanager.get(po.eventRecord.keyboardDomEventDataArrMap, eventName);
  if (arr !== undefined) {
