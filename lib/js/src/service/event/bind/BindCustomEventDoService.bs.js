...
var MutableHashMap$WonderEventmanager = require("../../../structure/hash_map/MutableHashMap.bs.js");
...
function _addToEventArr(eventName, eventData, eventArrMap) {
  var arr = MutableHashMap$WonderEventmanager.get(eventArrMap, eventName);
  if (arr !== undefined) {
    return MutableHashMap$WonderEventmanager.set(eventArrMap, eventName, _addEventDataByPriority(eventData, arr));
  } else {
    return MutableHashMap$WonderEventmanager.set(eventArrMap, eventName, [eventData]);
  }
}
...
function _removeFromEventArrMapByHandleFunc(eventName, handleFunc, eventArrMap) {
  var arr = MutableHashMap$WonderEventmanager.get(eventArrMap, eventName);
  if (arr !== undefined) {
    return MutableHashMap$WonderEventmanager.set(eventArrMap, eventName, _removeFromEventArrByHandleFunc(arr, handleFunc));
  } else {
    return eventArrMap;
  }
}
...
function _removeFromEventListMapByEventName(eventName, eventArrMap) {
  return MutableHashMap$WonderEventmanager.deleteVal(eventArrMap, eventName);
}
...
function unbindGlobalEventByEventName(eventName, po) {
  ...
  return {
          eventRecord: {
            ...
            keyboardDomEventDataArrMap: eventRecord.keyboardDomEventDataArrMap,
            touchDomEventDataArrMap: eventRecord.touchDomEventDataArrMap,
            customGlobalEventArrMap: MutableHashMap$WonderEventmanager.deleteVal(eventRecord.customGlobalEventArrMap, eventName),
            customGameObjectEventArrMap: eventRecord.customGameObjectEventArrMap,
            mouseEventData: eventRecord.mouseEventData,
            keyboardEventData: eventRecord.keyboardEventData,
            touchEventData: eventRecord.touchEventData
