...
var Exception$WonderEventmanager = require("./Exception.bs.js");
...
var _buildErr = Exception$WonderEventmanager.buildErr;

function failWith(x) {
  return {
          TAG: /* Error */1,
          _0: Exception$WonderEventmanager.buildErr(x)
        };
}
...
function tryCatch(oneTrackFunc) {
  try {
    ...
    if (e.RE_EXN_ID === Js_exn.$$Error) {
      ...
      return {
              TAG: /* Error */1,
              _0: Exception$WonderEventmanager.buildErr("unknown error: " + e)
            };
    }
  }
}
...
function getExn(result) {
  return handleFail(result, Exception$WonderEventmanager.throwErr);
}
