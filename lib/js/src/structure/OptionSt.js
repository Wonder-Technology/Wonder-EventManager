...
function buildFailResult(param) {
  return Result$WonderEventmanager.failWith("data not exist in option data");
}

function get(optionData) {
  if (optionData !== undefined) {
    return Result$WonderEventmanager.succeed(Caml_option.valFromOption(optionData));
  } else {
    return Result$WonderEventmanager.failWith("data not exist in option data");
  }
}
...
function forEachResult(optionData, func) {
  if (optionData !== undefined) {
    ...
    return Result$WonderEventmanager.succeed(undefined);
  }
}

function sequenceResultM(optionData) {
  if (optionData !== undefined) {
    return Result$WonderEventmanager.mapSuccess(optionData, (function (value) {
