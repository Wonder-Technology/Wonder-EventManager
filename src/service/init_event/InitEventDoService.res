...
let _unsafeGetBody = po => po->BodyDoService.unsafeGetBody->bodyToEventTarget
...
let _fromPointDomEvent = (eventName, po) =>
  WonderBsMost.Most.fromEvent(eventName, _unsafeGetBody(po), false)

let _fromMobilePointDomEvent = (eventName, po) => {
  WonderBsMost.Most.fromEvent(eventName, _unsafeGetBody(po), {"passive": false}->Obj.magic)
}
